...
address = settings.erc20_contract_address
contract = web3.eth.contract(address=address, abi=abi)
token_decimal = contract.functions.decimals().call()


def get_total_supply():
    return contract.functions.totalSupply().call()/(10**token_decimal)
...
def get_acct_balance(public_key:str, as_erc20=False): # can fail!
    if as_erc20:
        return contract.functions.balanceOf(web3.toChecksumAddress(public_key)).call()/10**token_decimal
    else:
...
def send_erc20(from_address:str, to_address:str, private_key:str, amount:float, max_gas_gwei:float=250, gas_fee_gwei:float=3.0, chainid=5, verify_balance=False):
    ...
    if verify_balance:
        ...
        if balance < wei_amount:
            raise ValidationError("insufficient erc20 token!")
    if web3.eth.get_balance(from_address) < max_gas_gwei:
        raise ValidationError("insufficient ether to be used as gas fee!")
    ...
    tx = {
        ...
        'to': address,
        'value': "0x0",
        'maxFeePerGas': max_gas_gwei,
        'maxPriorityFeePerGas': gas_fee_gwei,
        'chainId': chainid,
        "data": contract.encodeABI('transfer', args=(to_address, wei_amount))
        }
